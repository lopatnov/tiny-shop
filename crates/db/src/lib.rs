...
///
/// # ⚠️ Deadlock внутри транзакции
/// `writer` имеет `max_connections(1)`. Внутри активной транзакции (`writer.begin()` заняла
/// единственное соединение) **нельзя** запрашивать второе соединение из того же пула — это
/// зависнет до `busy_timeout`. Правило: во все вложенные операции передавай **исполнитель
/// транзакции** (`&mut *tx`), а НЕ `&writer`. Так, `outbox::enqueue(&mut *tx, ev)` — верно;
/// `outbox::enqueue(&db.writer, ev)` внутри открытой транзакции — нет.
#[derive(Clone)]
